...
#  endif
    ...
    public:
		...
		void temp_cutoutblock2
			(std::vector<Parser::Token> const& tokens, ConfigT& config,
			 Parser::Token const& token, Arg const& arg, std::vector<std::string>& errors, std::size_t& i) const
		{
			try {
		...
		void temp_cutoutblock
		(std::vector<Parser::Token> const& tokens, ConfigT& config,
				std::vector<Parser::Token>& unusedTokens, std::vector<std::string>& errors) const {
            ...
            for( std::size_t i = 0; i < tokens.size(); ++i ) {
                ...
                for(; it != itEnd; ++it ) {
                    Arg const& arg = *it;
					temp_cutoutblock2(tokens, config, token, arg, errors, i);
                }
                if( it == itEnd ) {
        ...
        std::vector<Parser::Token> populateOptions( std::vector<Parser::Token> const& tokens, ConfigT& config ) const {
			...
			temp_cutoutblock(tokens, config, unusedTokens, errors);
